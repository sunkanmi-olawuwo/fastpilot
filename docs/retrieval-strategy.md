...
FastPilot retrieves over a 4,232-chunk index built from 514 harmonized documents across four sources (`official_docs`, `template_repo`, `github_issue`, `github_discussion`). The production retrieval path is **hybrid search → Voyage rerank → top-10 to the LLM**, the configuration (T1b) that won the head-to-head. Every claim below is grounded in the pairwise evaluation (`week-3/evaluations/week3_comparison.md`), and the live parameters are read from `app/config.py`.

## Hybrid search

Each query is embedded two ways and fused:

- **Dense:** Voyage `voyage-4-lite`, 2048-d, into the `text-dense` named vector. Its 32K-token window eliminated the silent truncation a smaller embedder would cause, so it was kept unchanged from the chunking evaluation's winner.
...
In isolation, hybrid is roughly a wash: T1a (hybrid, no rerank) scored 22 pts vs the dense-only baseline's 24 — BM25's recall is cancelled by its noise floor on this corpus. Hybrid's real value is as the **wide candidate net the reranker operates on**, not as a better top-10 by itself.
...
**T1b (hybrid + Voyage rerank) is the production default.** In the 4-way pairwise eval (72 judge calls over 12 hold-out questions), T1b won **30 / 36** pairwise comparisons with **8 / 12 first-place finishes**, never finished last, and beat the dense-only baseline by 19 points (43 vs 24).

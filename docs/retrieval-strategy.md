...
Both branches **prefetch 100 candidates** (`dense_prefetch=100`, `sparse_prefetch=100`) and are combined with Qdrant-native **Reciprocal Rank Fusion** (`FusionQuery(fusion=Fusion.RRF)`, k=60 default — no score normalization, robust to the cosine-vs-BM25 scale mismatch). Explicit prefetch limits matter: Haystack's built-in hybrid retriever under-fetches (~10–20 docs/branch), starving the reranker's candidate pool. RRF emits its top **50** (`rerank_input=50`) — this is the candidate set handed to the reranker.
...
Reranking is **the single biggest accuracy lever in the study**: T1b's 43 points vs T1a's 22 is a **+21-point jump** — nearly doubling the score. It promoted the highest-direct-contribution chunk to rank 1 on 8/12 questions (vs 1/12 without it) and eliminated all last-place finishes, for a latency cost of ~50ms (+1.4%). If the Voyage call fails, the reranker falls back to the upstream RRF order truncated to `top_k`, so the pipeline survives reranker outages.
